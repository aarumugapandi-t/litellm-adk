...
async def run_hitl_astream_demo():
    print("--- Streaming HITL Demo (Async) ---")
    session_id = str(uuid.uuid4())
    user_input = "Refund $100 to user 999"
    print(f"\nUser: {user_input}")
    ...
    async for chunk in agent.astream(user_input, session_id=session_id, stream_events=True):
    ...
    if pending_request_id:
        ...
        async for chunk in agent.astream("", session_id=session_id, stream_events=True):
            if isinstance(chunk, dict):
                if chunk.get("type") == "content":
                    print(chunk.get("delta"), end="", flush=True)
...
if __name__ == "__main__":
    asyncio.run(run_hitl_astream_demo())
